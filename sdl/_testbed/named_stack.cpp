...
template <typename T> NamedStack<T>::~NamedStack()
{
	...
	for (typename _StacktypeMap::iterator it = _map.begin(); it != _map.end(); ++it)
		delete it->second;
	SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "NamedStack::~NamedStack()");
}
...
template <typename T> T* NamedStack<T>::add(string key)
{
	...
	{
		SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "NamedStack::add() - Key %s already exists.", key.c_str());
		return 0;
	}
	...
	if (key.length()==0)
	{
		SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "NamedStack::add() - Key lenth must be > 0");
		return 0;
	}
...
template <typename T> T* NamedStack<T>::push(string key)
{
	...
	if (exists)
	{
		SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "NamedStack::push() - Key %s already exists.", key.c_str());
		return 0;
	}

	// Add an element pointer to the top of the stack and return it
	cout << "Pushing " << key << " to top of stack" << endl;
...
template <typename T> void NamedStack<T>::pop()
{
	...
	if (size() <= 0)
	{
		SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION, "NamedStack::pop() - Stack is empty");
